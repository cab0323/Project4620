...
/*
 * This is your interface file. Note: I have not fully tested this yet, as time has been short.
 * Hopefully there are no major errors in it, and you should not need to make any major changes to
 * this file.
 * 
 * Also note, there is currently little to no data validation happening in this code, so when you
 * run it, choose your inputs carefully. Do not just test it on meaningless input, as it will affect
 * your database.
 * 
 * YOU DO NEED TO EDIT THE PROMPT FOR THE CUSTOMERS PHONE NUMBER, TO LET ME KNOW WHAT FORMAT IT
 * SHOULD BE (with dashes, or without, etc.)
 */
...
public class Menu {
  ...
  public static void EnterOrder() throws SQLException, IOException {
    ...
    if (answer == 1) {
      ...
      if (answer == 2) {
        ...
        newCust =
            new DeliveryCustomer(temp.getID(), temp.getName(), temp.getPhone(), temp.getAddress());
    ...
    while (more.equals("Y") || more.equals("y")) {
      ...
      if (chosen_d <= discs.size()) {
        ...
        Discount newD = new Discount(discs.get(chosen_d - 1).getName(),
            discs.get(chosen_d - 1).getPercentDisc(), discs.get(chosen_d - 1).getCashDisc(),
            discs.get(chosen_d - 1).getID());
  ...
  public static void EnterCustomer() throws SQLException, IOException {
     ...
     * 
     * 
     * Enter phone number in  dashed mode : 000 - 000 - 0000
     * 
     * 
     ***********************************************************************************/
  ...
  public static void ViewInventoryLevels() throws SQLException, IOException {
    ...
    for (Topping t : curInventory) {
      System.out.println(Integer.toString(t_count) + ": " + t.getName() + " Level: "
          + Double.toString(t.getInv()));
      t_count++;
    }
  ...
  public static Pizza buildPizza() throws SQLException, IOException {
    ...
    while (chosen_t != -1) {
      ...
      System.out.println(
          "Which topping do you want to add? Enter the number. Enter -1 to stop adding toppings: ");

      chosen_t = Integer.parseInt(reader.readLine());
      if (chosen_t != -1) {
        if (chosen_t <= curInventory.size()) {
          // make copy to avoid aliasing issues
          Topping newT = new Topping(curInventory.get(chosen_t - 1).getName(),
              curInventory.get(chosen_t - 1).getPrice(), curInventory.get(chosen_t - 1).getInv());
    ...
    if (yn.equals("Y") || yn.equals("y")) {
      ...
      while (chosen_d != -1) {
        ...
        System.out.println(
            "Which discount do you want to add? Enter the number. Enter -1 to stop adding discounts: ");
