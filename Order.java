...
/**
 * Order can hold any type of order, dine in, pick up or delivery. The order contains the pizzas,
 * customer and discounts
 *
 * @invariants ID = (a valid id || -1 to show a new order) cust can be any customer type order_type
 *             = (DBNinja.dine_in || DBNinja.pickup || DBNinja.delivery)
 *
 */
...
public class Order {
   ...
   * @requires i = (a valid id || -1 to show a new order) AND type = (DBNinja.dine_in ||
   *           DBNinja.pickup || DBNinja.delivery)
   * @ensures ID = i and cust = c and order_type = type and the order has not pizzas or discounts
   *          yet
   */
   ...
   * order to the format needed for your database
   * 
   ...
   * 
   * @return a string with the order number, type, customer information, number of pizzas and the
   *         price of the order
   */
  public String toSimplePrint() {
    ...
    s += " Number of Pizzas: " + Integer.toString(pizzas.size()) + " Price: $"
        + Double.toString(this.calcPrice()) + "\n";
    return s;
  }
