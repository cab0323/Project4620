...
/**
 * Pizza will hold all the relevant information such as ID, size, crust, toppings, discounts, and
 * the price of the pizza
 * 
 * @invariants ID = (a valid id || -1 to show a new order) size = (DBNinja.size_s || DBNinja.size_m
 *             || DBNinja.size_l || DBNinja.size_xl) crust = (DBNinja.crust_thin ||
 *             DBNinja.crust_orig || DBNinja.crust_pan || DBNinja.crust_gf) baseprice is the base
 *             price for that size and crust
 */
public class Pizza {
  private int ID;
  private String size;
   ...
   * @requirws i = (a valid id || -1 to show a new order) s = (DBNinja.size_s || DBNinja.size_m ||
   *           DBNinja.size_l || DBNinja.size_xl) c = (DBNinja.crust_thin || DBNinja.crust_orig ||
   *           DBNinja.crust_pan || DBNinja.crust_gf) bp is the base price for that size and crust
   *
   * @ensures ID = i AND size = s AND crust = c AND base_price = bp AND pizza starts with no
   *          toppings or discounts
   */
   ...
   * @ensures calcPrice is calculated using the base price, the price of each toppings, and any
   *          discounts
   */
  ...
  /*
   * Its bad practice to expose the arrays like this, but it will make it easier to convert the
   * order to the format needed for your database and I didn't have the time to go do this properly
   * 
